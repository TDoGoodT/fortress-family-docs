...
FROM core.ledger_contract_person_created c
ORDER BY c.event_timestamp, c.causation_id;
SQL
)"

if [ -z "${HANDOFF_IDS}" ]; then
  echo "No eligible person handoffs found in core.ledger_contract_person_created"
  exit 0
fi
...
BEGIN
    /*
      ...
      - no receipt => event/person existence means inconsistent partial state
    */
    SELECT EXISTS (
        SELECT 1
    ...
    */
    ...
    /*
      ...
      target person must not already exist without a receipt.
    */
    SELECT EXISTS (
        SELECT 1
        FROM core.person p
        WHERE p.person_id = v_contract.aggregate_id
    )
    INTO v_person_exists;

    IF v_person_exists THEN
        RAISE EXCEPTION
            'Inconsistent partial state: person already exists without receipt for handoff_request_id=% person_id=%',
            v_handoff_request_id,
    ...
    */
    ...
    /*
      ...
      materialize core.person from the inserted event via projection rule
    */
    ...
    */
    ...
    RAISE NOTICE 'Applied handoff_request_id=%, event_id=%, person_id=%',
        v_contract.causation_id,
        v_event_id,
        v_contract.aggregate_id;
END
