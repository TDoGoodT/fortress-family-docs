...
FROM core.ledger_contract_task_created c
ORDER BY c.event_timestamp, c.causation_id;
SQL
)"

if [ -z "${HANDOFF_IDS}" ]; then
  echo "No eligible task handoffs found in core.ledger_contract_task_created"
  exit 0
fi
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1
        FROM core.task t
        WHERE t.task_id = v_contract.aggregate_id
    )
    INTO v_task_exists;

    IF v_task_exists THEN
        RAISE EXCEPTION
            'Inconsistent partial state: task already exists without receipt for handoff_request_id=% task_id=%',
            v_handoff_request_id,
    ...
    RAISE NOTICE 'Applied handoff_request_id=%, event_id=%, task_id=%',
        v_contract.causation_id,
        v_event_id,
        v_contract.aggregate_id;
END
