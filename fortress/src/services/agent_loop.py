...
def build_system_prompt(db: Session, member: FamilyMember) -> str:
    ...
    parts += [
        ...
        "## הוראות לסוכן",
        "- תמיד ענה בעברית",
        "- השתמש בכלים לביצוע פעולות — אל תמציא נתונים",
        "- שמור על תשובות קצרות — זה וואטסאפ",
        "- כשהמשתמש מבקש עזרה, השתמש בכלי system_help",
        "- אל תטען שביצעת פעולה מבלי לקרוא לכלי המתאים",
        "- כשמציגים רשימות, קרא תחילה לכלי הרלוונטי",
    ]
    return "\n".join(parts)
